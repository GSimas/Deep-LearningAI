...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = np.divide(np.multiply(np.dot(e,g),g),np.square(np.linalg.norm(g)))
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = np.divide(np.multiply(np.dot(mu,bias_axis),bias_axis),np.square(np.linalg.norm(bias_axis)))
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = np.divide(np.multiply(np.dot(e_w1,bias_axis),bias_axis),np.square(np.linalg.norm(bias_axis)))
    e_w2B = np.divide(np.multiply(np.dot(e_w2,bias_axis),bias_axis),np.square(np.linalg.norm(bias_axis)))
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    corrected_e_w1B = np.multiply(np.sqrt(np.absolute(np.subtract(1,np.square(np.linalg.norm(mu_orth))))),np.divide(np.subtract(e_w1B,mu_B),np.absolute(np.subtract(np.subtract(e_w1,mu_orth),mu_B))))
    corrected_e_w2B = np.multiply(np.sqrt(np.absolute(np.subtract(1,np.square(np.linalg.norm(mu_orth))))),np.divide(np.subtract(e_w2B,mu_B),np.absolute(np.subtract(np.subtract(e_w2,mu_orth),mu_B))))
